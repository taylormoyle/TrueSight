...
class Neural_Network:
    ...
    def forward_prop(self, infos, inp, weights, training=False):
        ...
        batch_norm1 = op.batch_normalize(relu1, w['bnb1'], w['bng1'], mv['bn1'], training=True)

        '''   LAYER 2   '''
        conv2 = op.convolve(batch_norm1[0], w['conv2'], pad=pad2)
        relu2 = op.sigmoid(conv2)
        batch_norm2 = op.batch_normalize(relu2, w['bnb2'], w['bng2'], mv['bn2'], training=True)
        pool2 = op.pool(batch_norm2[0], p_h1, p_w1, stride1)

        '''   LAYER 3   '''
        conv3 = op.convolve(pool2, w['conv3'], pad=pad3)
        relu3 = op.sigmoid(conv3)
        batch_norm3 = op.batch_normalize(relu3, w['bnb3'], w['bng3'], mv['bn3'], training=True)
        """
        '''   LAYER 4   '''
    ...
    def init_facial_rec_weights(self, infos):
        ...
        weights['bnb1'] = op.initialize_weights((n_fm1,))
        weights['bng1'] = op.initialize_weights((n_fm1,))
        running_mean_var['bn1'] = [None, None]

        n_fm2, f_h, f_w, _, _ = infos[1]
        weights['conv2'] = op.initialize_weights((n_fm2, n_fm1, f_h, f_w))
        weights['bnb2'] = op.initialize_weights((n_fm2,))
        weights['bng2'] = op.initialize_weights((n_fm2,))
        running_mean_var['bn2'] = [None, None]

        n_fm3, f_h, f_w, _, _ = infos[3]
        weights['conv3'] = op.initialize_weights((n_fm3, n_fm2, f_h, f_w))
        weights['bnb3'] = op.initialize_weights((n_fm3,))
        weights['bng3'] = op.initialize_weights((n_fm3,))
        running_mean_var['bn3'] = [None, None]
        """
