...
class Neural_Network:
    ...
    def create_facial_rec(self, infos, inp):
        ...
        relu1 = op.relu(conv1)
        batch_norm1 = op.batch_normalize(relu1, w['bn1'])

        '''   LAYER 2   '''
        conv2 = op.convolve(batch_norm1, w['conv2'], pad=pad2)
        relu2 = op.relu(conv2)
        batch_norm2 = op.batch_normalize(relu2, w['bn2'])
        pool2 = op.pool(batch_norm2, p_h1, p_w1, stride1)

        '''   LAYER 3   '''
        conv3 = op.convolve(pool2, w['conv3'], pad=pad3)
        relu3 = op.relu(conv3)
        batch_norm3 = op.batch_normalize(relu3, w['bn3'])

        '''   LAYER 4   '''
        conv4 = op.convolve(batch_norm3, w['conv4'], pad=pad4)
        relu4 = op.relu(conv4)
        batch_norm4 = op.batch_normalize(relu4, w['bn4'])
        pool4 = op.pool(batch_norm4, p_h2, p_w2, stride2)

        '''   LAYER 5   '''
        conv5 = op.convolve(pool4, w['conv5'], pad=pad5)
        relu5 = op.relu(conv5)
        batch_norm5 = op.batch_normalize(relu5, w['bn5'])
        pool5 = op.pool(batch_norm5, p_h3, p_w3, stride3)

        '''   LAYER 6   '''
        conv6 = op.convolve(pool5, w['conv6'], pad=pad6)
        relu6 = op.relu(conv6)
        batch_norm6 = op.batch_normalize(relu6, w['bn6'])
        pool6 = op.pool(batch_norm6, p_h4, p_w4, stride4)

        '''   LAYER 7   '''
        conv7 = op.convolve(pool6, w['conv7'], pad=pad7)
        relu7 = op.relu(conv7)
        batch_norm7 = op.batch_normalize(relu7, w'bn7')
        pool7 = op.pool(batch_norm7, p_h5, p_w5, stride5)

        '''   LAYER 8   '''
        conv8 = op.convolve(pool7, w['conv8'], pad=pad8)
        relu8 = op.relu(conv8)
        batch_norm8 = op.batch_normalize(relu8, w['bn8'])

        '''   LAYER 9   '''
        conv9 = op.convolve(batch_norm8, w['conv9'], pad=pad9)
        relu9 = op.relu(conv9)
        batch_norm9 = op.batch_normalize(relu9, w['bn9'])

        '''   LAYER 10   '''
        conv10 = op.convolve(batch_norm9, w['conv10'], pad=pad10)
    ...
    def backward_prop(self, cache, output, labels, infos):
        ...
        '''   LAYER 9   '''
        batch_norm_dx, batch_norm9_db, batch_norm9_dg = op.batch_norm_backprop(conv_dx, cache[1][1])
        ...
        '''   LAYER 8   '''
        batch_norm_dx, batch_norm8_db, batch_norm8_dg = op.batch_norm_backprop(conv_dx, cache[3][1])
        ...
        batch_norm_dx, batch_norm7_db, batch_norm7_dg = op.batch_norm_backprop(pool, cache[6][1])
        ...
        batch_norm_dx, batch_norm5_db, batch_norm5_dg = op.batch_norm_backprop(pool, cache[12][1])
        ...
        batch_norm_dx, batch_norm4_db, batch_norm4_dg = op.batch_norm_backprop(pool, cache[15][1])
        relu_error = op.relu_prime(cache[16]) * batch_norm_dx
        ...
        '''   LAYER 3   '''
        batch_norm_dx, batch_norm3_db, batch_norm3_dg = op.batch_norm_backprop(conv_dx, cache[17][1])
        relu_error = op.relu_prime(cache[18]) * batch_norm_dx
        ...
        batch_norm_dx, batch_norm2_db, batch_norm2_dg = op.batch_norm_backprop(pool, cache[20][1])
        relu_error = op.relu_prime(cache[21]) * batch_norm_dx
        ...
        '''   LAYER 1   '''
        batch_norm_dx, batch_norm1_db, batch_norm1_dg = op.batch_norm_backprop(conv_dx, cache[22][1])
        relu_error = op.relu_prime(cache[23]) * batch_norm_dx
