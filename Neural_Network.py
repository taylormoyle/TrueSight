...
class Neural_Network:
    ...
    def forward_prop(self, infos, inp, weights, training=False):
        ...
        N, D, H, W = relu10.shape
        flatten = relu10.reshape(-1, D * H * W)
        ...
        if training:
            ...
            cache = {'c10': conv10, 'c9': conv9, 'c8': conv8, 'c7': conv7, 'c6': conv6, 'c5': conv5, 'c4': conv4,
                     ...
                     'p6': pool6, 'p5': pool5, 'p4': pool4, 'p2': pool2, 'inp': inp, 'fc': full_conn, 'fl': flatten}
    ...
    def backward_prop(self, cache, output, labels, weights, infos):
        ...
        full_dx = full_dx.reshape(cache['r10'].shape)
        ...
        '''   LAYER 4   '''
        pool = op.pool_backprop(conv_dx, cache['r4'], p_h2, p_w2, stride2)
